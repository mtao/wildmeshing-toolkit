...
void run(const std::array<int64_t, N>& arr)
{
    ...
    for (int8_t aperm = 0; aperm < sb.size(); ++aperm) {
        ...
        const auto Bperm = wmtk::dart::utils::get_local_vertex_permutation(pt, bperm);
        spdlog::warn("Resulting perm: {} ", fmt::join(Bperm, ","));
        CHECK(aperm == bperm);
    }
}
...
TEST_CASE("get_vertex_permutation", "[permutation]")
{
    ...
    const int8_t i2 =
        wmtk::dart::SimplexDart::get_singleton(wmtk::PrimitiveType::Triangle).identity();
    const int8_t i3 =
        wmtk::dart::SimplexDart::get_singleton(wmtk::PrimitiveType::Tetrahedron).identity();
    ...
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 4>{{10, 20, 100, 101}}) ==
        i3);
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 4>{{10, 20, 95, 101}}) ==
        i3);

    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 2>{{20, 5}}) ==
        d10.permutation());
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 3>{{20, 5, 30}}) ==
        d102.permutation());
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 3>{{20, 30, 5}}) ==
        d120.permutation());
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 3>{{30, 20, 5}}) ==
        d210.permutation());
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 4>{{5, 4, 3, 2}}) ==
        d3210.permutation());
    CHECK(
        wmtk::dart::utils::from_vertex_permutation(std::array<int64_t, 4>{{95, 20, 10, 101}}) ==
        d2103.permutation());

    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 2>{{5, 20}},
            std::array<int64_t, 2>{{5, 20}}) == i1);
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 3>{{5, 20, 30}},
            std::array<int64_t, 3>{{5, 20, 30}}) == i2);
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 3>{{10, 20, 100}},
            std::array<int64_t, 3>{{10, 20, 100}}) == i2);
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 4>{{10, 20, 100, 101}},
            std::array<int64_t, 4>{{10, 20, 100, 101}}) == i3);
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 4>{{10, 20, 95, 101}},
            std::array<int64_t, 4>{{10, 20, 95, 101}}) == i3);

    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 2>{{5, 20}},
            std::array<int64_t, 2>{{20, 5}}) == d10.permutation());
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 3>{{5, 20, 30}},
            std::array<int64_t, 3>{{20, 5, 30}}) == d102.permutation());
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 3>{{5, 20, 30}},
            std::array<int64_t, 3>{{20, 30, 5}}) == d120.permutation());
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 3>{{5, 20, 30}},
            std::array<int64_t, 3>{{30, 20, 5}}) == d210.permutation());

    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 4>{{2, 3, 4, 5}},
            std::array<int64_t, 4>{{5, 4, 3, 2}}) == d3210.permutation());
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 4>{{10, 20, 95, 101}},
            std::array<int64_t, 4>{{95, 20, 10, 101}}) == d2103.permutation());
    CHECK(
        wmtk::dart::utils::get_permutation(
            std::array<int64_t, 4>{{10, 20, 95, 101}},
            std::array<int64_t, 4>{{95, 10, 20, 101}}) == d2013.permutation());
