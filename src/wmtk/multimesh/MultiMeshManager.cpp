    ...
    if (true) {
        ...
        if (target_pt == min_pt) {
            ...
            logger().warn(
                ...
                osd.tuple_from_dart(involution).as_string(),
                ...
                sd.tuple_from_dart(inverse_involution).as_string(),
    ...
    const std::vector<std::array<Tuple, 2>>& child_tuple_my_tuple_map)
{
    ...
    assert(
        child_tuple_my_tuple_map.size() ==
        child_mesh_ptr->get_all(child_mesh_ptr->top_simplex_type()).size());
    // register maps
