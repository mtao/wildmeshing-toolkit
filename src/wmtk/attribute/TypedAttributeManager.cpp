...
template <typename T>
void TypedAttributeManager<T>::serialize(const int dim, io::MeshWriter& writer) const
{
...
{
    auto it = m_handles.find(name);
    return it != m_handles.end() && has_attribute(it->second);
}

template <typename T>
bool TypedAttributeManager<T>::has_attribute(const AttributeHandle& h) const
{
