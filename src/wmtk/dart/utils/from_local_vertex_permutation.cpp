...
int8_t from_vertex_permutation(Eigen::Ref<const VectorX<int64_t>> indices)
{
    ...
    if (map.size() != indices.size()) {
        throw std::runtime_error(fmt::format(
            "duplicated indices in from_vertex_permutation {}",
            fmt::join(indices, ",")));
    }
...
template int8_t from_local_vertex_permutation<2>(const std::array<int8_t, 2>& indices);
template int8_t from_local_vertex_permutation<3>(const std::array<int8_t, 3>& indices);
template int8_t from_local_vertex_permutation<4>(const std::array<int8_t, 4>& indices);
