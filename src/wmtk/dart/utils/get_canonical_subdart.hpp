...
inline int8_t get_canonical_subdart(const SimplexDart& sd, PrimitiveType pt, int8_t permutation)
{
    const PrimitiveType source_pt = sd.simplex_type();

    if (source_pt == PrimitiveType::Edge) {
        if (pt == PrimitiveType::Vertex) {
            return autogen::edge_mesh::canonical_subdart[get_primitive_type_id(pt)][permutation];
        }
    } else if (source_pt == PrimitiveType::Triangle) {
        if (pt <= PrimitiveType::Edge) {
            return autogen::tri_mesh::canonical_subdart[get_primitive_type_id(pt)][permutation];
        }
    } else if (source_pt == PrimitiveType::Tetrahedron) {
        if (pt <= PrimitiveType::Triangle) {
            return autogen::tet_mesh::canonical_subdart[get_primitive_type_id(pt)][permutation];
        }
    }
    return sd.identity();
}
